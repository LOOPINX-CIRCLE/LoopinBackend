...
**Field Explanations:**
- `onesignal_player_id` (string, required): **Generated by OneSignal SDK on the phone application (client-side)**, not by the backend. When you initialize OneSignal SDK in your mobile app, it automatically generates a unique player ID for that specific device. You retrieve this ID using OneSignal SDK methods (e.g., `OneSignal.getDeviceState()?.userId` in iOS or `OneSignal.shared.getDeviceState()?.userId` in Flutter) and then send it to the backend. The backend only receives and stores this ID - it does NOT generate it. Each device gets a unique player ID that persists until the app is uninstalled or the device is reset.
- `platform` (string, required): Either `"ios"` or `"android"`. Use `"ios"` for iOS devices and `"android"` for Android devices. This tells the backend which platform the device is using.

**Important:** The `onesignal_player_id` is **NOT generated by the backend**. It is generated by the OneSignal SDK on your phone application when you initialize OneSignal. You must:
1. Initialize OneSignal SDK in your app (see platform-specific sections below)
2. Request notification permissions from the user
3. Retrieve the player ID from OneSignal SDK using the SDK's methods
4. Send that player ID to this backend endpoint to register the device

The backend never generates player IDs - it only stores the player ID that your app sends.
